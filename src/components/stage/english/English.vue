<template>
    <div>
        ...
        <el-dialog
            ...
            <EnglishStage v-on:refresh="refresh()" :stageData="stageData"></EnglishStage>
            <span slot="footer" class="dialog-footer">
...
<script>
    import EnglishStage from "./EnglishStage"
    export default {
        name: "English",
        components:{
            EnglishStage
        ...
        mounted:function(){
            var this_=this;
            this.getData();
            this.baseFun().axios("post",this.baseUrl().book_press_version,{subject_id:9},function (res) {
        ...
        methods:{
            ...
            refresh:function(){//请求以及刷新stageData数据
                var this_=this;
            ...
            getData:function(){//获取主页数据列表
                var this_=this;
                this.baseFun().axiosGet(this.baseUrl().book_home+"?subject_id=9",function(res){
                    //console.log(res)
            ...
            handleStatus(index, row){//点击关卡管理按钮
                ...
                if(row.status==="启用"){
                    ...
                    this.$confirm('此操作将禁用关卡, 是否继续?', '提示', {
                        ...
                        this.baseFun().axios("post",this.baseUrl().book_home_status,{subject_id:9,book_id:row.book_id,status:status},function (res) {
            ...
            handleDelete(index, row){//删除教材
                ...
                this.$confirm('此操作将永久删除该教材, 是否继续?', '提示', {
                    ...
                    //console.log(row);
                    this.baseFun().axios("post",this.baseUrl().book_delete,{subject_id:9,book_id:row.book_id},function (res) {
                        //console.log(res)
                        if(res.data.success===true){
                            this_.$message({
                                message: '删除成功！',
