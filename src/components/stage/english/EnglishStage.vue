            ...
            <el-form :model="addStageData" label-width="50px" class="demo-ruleForm">
                ...
                <el-form-item label="类型">
                    <el-radio-group v-model="addStageData.section_type">
                        <el-radio label="1">单词跟读</el-radio>
                        <el-radio label="2">课文跟读</el-radio>
    ...
    export default {
        ...
        methods:{
            chaMoveUP:function(item){//目录上移
                //console.log(item);
                var this_=this;
                this.baseFun().axios("post",this.baseUrl().ChaMove_eng,{book_id:this.stageData.book_id,chapter_id:item.chapter_id,sequence:item.sequence,action:"up"},function (res) {
                    if(res.data.success){
                        this_.$message({
                            message: '移动成功！',
                            type: 'success'
            ...
            chaMoveDown:function(item){//目录下移
                //console.log(item);
                var this_=this;
                this.baseFun().axios("post",this.baseUrl().ChaMove_eng,{book_id:this.stageData.book_id,chapter_id:item.chapter_id,sequence:item.sequence,action:"down"},function (res) {
                    if(res.data.success){
                        this_.$message({
                            message: '移动成功！',
                            type: 'success'
            ...
            chaDelete:function(item){//删除目录
                ...
                this.$confirm('此操作将永久删除该目录, 是否继续?', '提示', {
                    ...
                    this.baseFun().axios("post",this.baseUrl().deleteCha_eng,{chapter_id:item.chapter_id},function (res) {
            ...
            chaOn:function (item) {//启用/禁用目录
                ...
                var this_=this;
                if(item.status===2){
                    this.baseFun().axios("post",this.baseUrl().statusCha_eng,{status:1,chapter_id:item.chapter_id},function (res) {
                        if(res.data.success===true){
                            this_.$message({
                                message: res.data.msg,
                                type: 'success'
                    ...
                    this.$confirm('此操作将禁用关卡, 是否继续?', '提示', {
                        ...
                        this.baseFun().axios("post",this.baseUrl().statusCha_eng,{status:2,chapter_id:item.chapter_id},function (res) {
                            if(res.data.success===true){
                                this_.$message({
                                    message: res.data.msg,
                                    type: 'success'
            ...
            ChaAdd:function(){//提交添加目录
                ...
                var this_=this;
                this.baseFun().axios("post",this.baseUrl().chaAdd_eng,this.addChaData,function (res) {
                    console.log(res);
                    if(res.data.success){
                        this_.$message({
                            message: '添加成功！',
            ...
            editCha:function(){//提交编辑过的目录信息
                ...
                var this_=this;
                this.baseFun().axios("post",this.baseUrl().editCha_eng,this.editChaData,function (res) {
                    console.log(res);
                    if(res.data.success){
                        this_.$message({
                            message: '修改成功！',
            ...
            stageMoveUp:function(s,item){//关卡上移
                ...
                var this_=this
                this.baseFun().axios("post",this.baseUrl().stageMove_eng,{section_id:s.section_id,book_id:this.stageData.book_id,chapter_id:item.chapter_id,sequence:s.sequence,action:"up"},function(res){
                    if(res.data.success){
                        this_.$message({
                            message: '移动成功！',
                            type: 'success'
            ...
            stageMoveDown:function(s,item){//关卡下移
                var this_=this
                this.baseFun().axios("post",this.baseUrl().stageMove_eng,{section_id:s.section_id,book_id:this.stageData.book_id,chapter_id:item.chapter_id,sequence:s.sequence,action:"down"},function(res){
                    if(res.data.success){
                        this_.$message({
                            message: '移动成功！',
                            type: 'success'
            ...
            stageContentClick:function(s,item){//点击关卡内容的名字
                var this_=this
                console.log(s,item);
                this.baseFun().axiosGet(this.baseUrl().stageContentEdit_eng+"?section_id="+s.section_id,function (res) {
            ...
            editStage:function(){
                var this_=this;
                this.baseFun().axios("post",this.baseUrl().stageEdit_eng,this.editStageData,function (res) {
                    if(res.data.success){
                        this_.$message({
                            message: '修改成功！',
                            type: 'success'
            ...
            updataStangeContent:function(){
                ...
                this.baseFun().axios("post",this.baseUrl().stageContentEdit_eng,this.editStageContentData,function (res) {
                    if(res.data.success){
                        this_.$message({
                            message: '更新成功！',
                            type: 'success'
            ...
            stageDelete:function(s,item){//删除关卡
                ...
                this.$confirm('此操作将永久删除该目录, 是否继续?', '提示', {
                    ...
                    this.baseFun().axios("post",this.baseUrl().stageDelete_eng,{chapter_id:item.chapter_id,section_id:s.section_id},function (res) {
                        if(res.data.success===true){
                            this_.$message({
                                message: '删除成功！',
                                type: 'success'
            ...
            stageAdd:function(){//提交添加关卡按钮
                ...
                this.baseFun().axios("post",this.baseUrl().stageAdd_eng,this.addStageData,function (res) {
                    console.log(res);
                    if(res.data.success){
                        this_.$message({
                            message: '添加成功！',
